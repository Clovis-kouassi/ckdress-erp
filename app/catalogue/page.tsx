            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 12 }}>
              {produitsAffiches.map(({ produit, stock }) =>
                stock.map(s => {
                  ...
                  return (
                    <div key={s.id} style={{ background: '#fff', border: isSelected ? '2.5px solid #1a1a1a' : '1.5px solid #ece9e3', borderRadius: 14, overflow: 'hidden', position: 'relative', transition: 'border-color 0.12s', boxShadow: isSelected ? '0 4px 16px rgba(0,0,0,0.12)' : '0 1px 4px rgba(0,0,0,0.06)' }}>
                      ...
                      <div style={{ padding: '10px 12px 12px' }}>
                        ...
                        {/* ✅ SÉLECTEUR QUANTITÉ — visible seulement si sélectionné */}
                        {isSelected && (
                          <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#f8f9fa', borderRadius: 10, padding: '6px 8px', border: '1px solid #e5e7eb' }}>
                            ...
                            <button
                              ...
                              style={{ width: 28, height: 28, borderRadius: 8, border: '1.5px solid #e5e7eb', background: '#fff', cursor: selItem.quantite >= selItem.maxQuantite ? 'not-allowed' : 'pointer', fontSize: 18, fontWeight: 700, color: selItem.quantite >= selItem.maxQuantite ? '#ccc' : '#1D9E75', display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 }}>
                              +
                            </button>
                          </div>
                        )}

                        {/* Bouton Commander individuel */}
                        {!isSelected ? (
