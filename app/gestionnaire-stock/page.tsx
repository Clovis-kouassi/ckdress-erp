    ...
    <div style={{ minHeight: '100vh', background: '#f4f4f5', fontFamily: 'sans-serif', color: '#1a1a1a' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #e5e5e5', padding: '12px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        <div>
          ...
          <p style={{ color: '#aaa', margin: '2px 0 0', fontSize: '11px' }}>{user?.nom} — {user?.activite}</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '10px', padding: '16px 16px 0' }}>
        {[
          ...
          { label: 'Références', value: produits.length, color: '#1a1a1a' },
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid #e5e5e5', background: '#fff', margin: '16px 0 0', overflowX: 'auto' }}>
        {[
          ...
          <button key={o.key} onClick={() => setOnglet(o.key as any)}
            style={{ flexShrink: 0, padding: '12px 16px', background: 'transparent', border: 'none', color: onglet === o.key ? '#0891b2' : '#888', fontSize: '13px', fontWeight: 600, cursor: 'pointer', borderBottom: onglet === o.key ? '2px solid #0891b2' : '2px solid transparent' }}>
            {o.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: '16px' }}>
        ...
        {onglet === 'stock' && (
          <div>
            {stockCritique.length > 0 && (
              <div style={{ background: '#fff5f5', border: '1px solid #E24B4A', borderRadius: '8px', padding: '10px 16px', marginBottom: '12px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '10px' }}>
              {stock.map(item => (
                <div key={item.id} style={{ background: '#fff', border: `1px solid ${item.quantite <= 3 ? '#E24B4A44' : '#e5e5e5'}`, borderRadius: '10px', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                  ...
                  <div style={{ padding: '10px 12px' }}>
                    <p style={{ margin: '0 0 4px', fontWeight: 600, fontSize: '13px', color: '#1a1a1a' }}>{(item.produits as any)?.nom}</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '12px' }}>
            {produits.map(prod => (
              <div key={prod.id} style={{ background: '#fff', border: '1px solid #e5e5e5', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                <div style={{ height: '160px', background: '#f5f5f5', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {prod.image_url
        ...
        {onglet === 'nouveau_produit' && (
          <div style={{ maxWidth: '600px' }}>
            <div style={{ background: '#fff', border: '1px solid #0891b244', borderRadius: '12px', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
              <h3 style={{ margin: '0 0 8px', fontSize: '15px', color: '#0891b2' }}>➕ Publier un nouveau produit</h3>
              <p style={{ margin: '0 0 16px', fontSize: '12px', color: '#aaa' }}>Ce produit sera visible dans le catalogue client.</p>
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <button onClick={() => fileRef.current?.click()}
                    style={{ padding: '10px 16px', borderRadius: '8px', border: '1.5px dashed #0891b2', background: '#f0f9ff', color: '#0891b2', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
                    📷 Choisir une photo
                  </button>
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                  ...
                  <button onClick={() => setCouleurs(prev => [...prev, nouvelleCouleur()])}
                    style={{ background: '#f0f9ff', color: '#0891b2', border: '1px solid #0891b244', borderRadius: '6px', padding: '4px 12px', fontSize: '12px', cursor: 'pointer' }}>
                    + Couleur
                  </button>
                </div>
                {couleurs.map((c, ci) => (
                  <div key={ci} style={{ background: '#f9f9f9', borderRadius: '10px', padding: '14px', marginBottom: '10px', border: '1px solid #f0f0f0' }}>
                    <div style={{ display: 'flex', gap: '10px', marginBottom: '10px', alignItems: 'center' }}>
                      ...
                      <button onClick={() => {
                        ...
                        input.click()
                      }} style={{ padding: '8px 12px', borderRadius: '6px', border: '1px dashed #0891b2', background: '#f0f9ff', color: '#0891b2', fontSize: '12px', cursor: 'pointer' }}>📷</button>
                      ...
                      {couleurs.length > 1 && (
                        <button onClick={() => setCouleurs(prev => prev.filter((_, i) => i !== ci))}
                          style={{ background: '#fff5f5', color: '#E24B4A', border: 'none', borderRadius: '6px', padding: '6px 10px', fontSize: '12px', cursor: 'pointer' }}>✕</button>
                      )}
                    </div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                      {c.tailles.map((t, ti) => (
                        <div key={t.taille} onClick={() => updateTailleCouleur(ci, ti, 'active', !t.active)}
                          style={{ display: 'flex', alignItems: 'center', gap: '4px', background: t.active ? '#0891b2' : '#fff', border: `1px solid ${t.active ? '#0891b2' : '#e5e5e5'}`, borderRadius: '6px', padding: '5px 8px', cursor: 'pointer' }}>
                          <span style={{ fontSize: '12px', fontWeight: 600, color: t.active ? 'white' : '#888' }}>{t.taille}</span>
                          {t.active && (
              ...
              <button onClick={publierProduit} disabled={saving}
                style={{ width: '100%', padding: '14px', borderRadius: '8px', background: saving ? '#aaa' : '#0891b2', border: 'none', color: 'white', fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', fontSize: '15px', marginTop: '16px' }}>
        ...
        {onglet === 'approvisionner' && (
          <div style={{ background: '#fff', border: '1px solid #e5e5e5', borderRadius: '12px', padding: '20px', maxWidth: '500px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
        ...
        {onglet === 'historique' && (
          <div>
            <h3 style={{ margin: '0 0 14px', fontSize: '13px', color: '#aaa', textTransform: 'uppercase' }}>📋 Dernières ventes boutiques</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {mouvements.map(v => (
                <div key={v.id} style={{ background: '#fff', border: '1px solid #e5e5e5', borderRadius: '10px', padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                  <div>
                    <p style={{ margin: 0, fontWeight: 600, fontSize: '13px', color: '#1a1a1a' }}>{v.nom_produit}</p>
