...
export default function GestionnaireStockPage() {
  ...
  const ouvrirCommande = async (cmd: any) => { setCommandeDetail(cmd); const ids = (cmd.variantes || '').split(',').map((v: string) => v.trim()).filter(Boolean); if (ids.length > 0) { const { data: stockItems } = await supabase.from('stock').select('*').in('id', ids); const { data: prodData } = await supabase.from('produits').select('image_url').eq('reference', cmd.produit_ref).single(); const items = (stockItems || []).map((s: any) => ({ ...s, image_url: s.image_url || prodData?.image_url || null })); setCommandeVariantesImages(items) } else { setCommandeVariantesImages([]) } }; const changerStatutCommande = async (id: string, statut: string) => {
