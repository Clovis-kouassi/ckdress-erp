...
export default function LivreursPage() {
  ...
  const generateCode = (index: number) => `LIV-${String(index).padStart(3, '0')}`

  const addLivreur = async () => {
    if (!nom || !telephone) return
    setSaving(true)
    const code = generateCode(livreurs.length + 1)
    ...
    <div style={{ background: '#0a0a0a', minHeight: '100vh', fontFamily: 'sans-serif', padding: '24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          ...
          <p style={{ color: '#555', margin: '4px 0 0', fontSize: '13px' }}>{livreurs.length} livreur(s) enregistré(s)</p>
        </div>
        <button
            ...
            <div key={livreur.id} style={{
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '4px' }}>
                  ...
                  <span style={{
                    background: livreur.actif ? '#1a2e25' : '#1a1a1a',
                    border: `1px solid ${livreur.actif ? '#1D9E75' : '#333'}`,
                    color: livreur.actif ? '#1D9E75' : '#555',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                ...
                <div style={{ background: '#0a1a12', border: '1px solid #1a3a28', borderRadius: '8px', padding: '8px 12px' }}>
                  ...
                  <p style={{ color: '#1D9E75', margin: 0, fontSize: '12px', fontFamily: 'monospace' }}>
                    /livreur/{livreur.code}
                  </p>
                </div>

                {/* Bouton copier lien */}
                <button
                  onClick={() => navigator.clipboard.writeText(`${baseUrl}/livreur/${livreur.code}`)}
                  style={{ padding: '8px 14px', background: 'transparent', border: '1px solid #333', color: '#888', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}
                >
                  📋 Copier
                </button>
                ...
                {/* Toggle actif */}
                <button
                  onClick={() => toggleActif(livreur.id, livreur.actif)}
