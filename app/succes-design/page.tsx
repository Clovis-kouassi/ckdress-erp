...
export default function SuccesDesignPage() {
  ...
  const [prodForm, setProdForm] = useState({
    reference: '', nom: '', categorie: '', prix_vente: 0, prix_achat: 0,
    description: '', image_file: null as File | null, preview: '', fournisseur_id: ''
  })
  ...
  const publierProduit = async () => {
    ...
    for (const couleur of couleurs) {
      ...
      if (couleur.image_file) {
        couleurImageUrl = await uploadImage(couleur.image_file, `succes-design/stock/${Date.now()}-${couleur.image_file.name}`) || ''
      }
    ...
    // Enregistrer l'achat si fournisseur sélectionné
    if (prodForm.fournisseur_id && prodForm.prix_achat) {
      ...
      await supabase.from('achats_fournisseur').insert({
        fournisseur_id: prodForm.fournisseur_id,
        produit_ref: prodForm.reference,
        nom_produit: prodForm.nom,
        quantite: totalQte,
        prix_achat_unitaire: prodForm.prix_achat,
        cout_total: totalQte * prodForm.prix_achat,
        activite: 'succes_design',
      })
    }

    setSuccess('✅ Produit publié dans le catalogue Succès Design !')
  ...
  const enregistrerAchat = async () => {
    ...
    await supabase.from('achats_fournisseur').insert({
      ...achatForm,
      cout_total: achatForm.quantite * achatForm.prix_achat_unitaire,
      activite: 'succes_design',
    })
    ...
    <div style={{ minHeight: '100vh', background: '#0a0a0a', fontFamily: 'sans-serif', color: 'white' }}>
      ...
      <div style={{ background: '#111', borderBottom: '1px solid #222', padding: '12px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ color: '#555', margin: '2px 0 0', fontSize: '11px' }}>{user?.nom} — Tenues importées</p>
        </div>
        <div style={{ display: 'flex', gap: '10px' }}>
          <a href="/succes-design/catalogue" target="_blank"
            style={{ background: 'rgba(212,168,83,0.1)', border: '0.5px solid #d4a853', borderRadius: '6px', color: '#d4a853', padding: '5px 12px', fontSize: '11px', cursor: 'pointer', textDecoration: 'none' }}>
            🌐 Voir catalogue
          </a>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '10px', padding: '16px 16px 0' }}>
        {[
          ...
          { label: 'Total produits', value: produits.length, color: 'white' },
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid #222', background: '#111', margin: '16px 0 0', overflowX: 'auto' }}>
        {[
          ...
          <button key={o.key} onClick={() => setOnglet(o.key as any)}
            style={{ flexShrink: 0, padding: '12px 16px', background: 'transparent', border: 'none', color: onglet === o.key ? '#d4a853' : '#555', fontSize: '13px', fontWeight: 600, cursor: 'pointer', borderBottom: onglet === o.key ? '2px solid #d4a853' : '2px solid transparent' }}>
            {o.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '12px' }}>
            {produits.length === 0 ? (
              ...
              <div key={prod.id} style={{ background: '#111', border: '1px solid #222', borderRadius: '12px', overflow: 'hidden' }}>
                <div style={{ height: '180px', background: '#1a1a1a', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {prod.image_url
                ...
                <div style={{ padding: '12px' }}>
                  ...
                  {prod.description && <p style={{ margin: '0 0 6px', color: '#666', fontSize: '11px' }}>{prod.description}</p>}
        ...
        {onglet === 'publier' && (
          <div style={{ maxWidth: '600px' }}>
            <div style={{ background: '#111', border: '1px solid #d4a853', borderRadius: '12px', padding: '20px' }}>
              <h3 style={{ margin: '0 0 16px', fontSize: '15px', color: '#d4a853' }}>➕ Publier un nouveau produit</h3>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
                {[
                  { key: 'reference', label: 'Référence *', placeholder: 'Ex: SD-001' },
                  { key: 'nom', label: 'Nom *', placeholder: 'Ex: Robe Ankara' },
                ].map(f => (
                  <div key={f.key}>
                ...
                <div>
                  ...
                  <input value={prodForm.categorie} onChange={e => setProdForm(p => ({ ...p, categorie: e.target.value }))}
                    placeholder="Ex: robe, ensemble, boubou"
                    style={{ width: '100%', padding: '10px', borderRadius: '8px', background: '#1a1a1a', border: '1px solid #333', color: 'white', fontSize: '13px', boxSizing: 'border-box' }} />
                </div>
                <div>
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <button onClick={() => fileRef.current?.click()}
                    style={{ padding: '10px 16px', borderRadius: '8px', border: '1.5px dashed #d4a853', background: 'rgba(212,168,83,0.1)', color: '#d4a853', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
                    📷 Choisir une photo
                  </button>
                  {prodForm.preview && <img src={prodForm.preview} style={{ width: '60px', height: '60px', objectFit: 'cover', borderRadius: '8px', border: '1px solid #333' }} />}
                </div>
              </div>
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                  ...
                  <button onClick={() => setCouleurs(prev => [...prev, nouvelleCouleur()])}
                    style={{ background: 'rgba(212,168,83,0.1)', color: '#d4a853', border: '1px solid #d4a85344', borderRadius: '6px', padding: '4px 12px', fontSize: '12px', cursor: 'pointer' }}>
                    + Couleur
                  </button>
                </div>
                {couleurs.map((c, ci) => (
                  <div key={ci} style={{ background: '#1a1a1a', borderRadius: '10px', padding: '14px', marginBottom: '10px' }}>
                    <div style={{ display: 'flex', gap: '10px', marginBottom: '10px', alignItems: 'center' }}>
                      ...
                      <button onClick={() => {
                        ...
                        input.click()
                      }} style={{ padding: '8px 12px', borderRadius: '6px', border: '1px dashed #d4a853', background: 'rgba(212,168,83,0.1)', color: '#d4a853', fontSize: '12px', cursor: 'pointer' }}>
                        📷
                      </button>
                      ...
                      {couleurs.length > 1 && (
                        <button onClick={() => setCouleurs(prev => prev.filter((_, i) => i !== ci))}
                          style={{ background: '#2a1010', color: '#E24B4A', border: 'none', borderRadius: '6px', padding: '6px 10px', fontSize: '12px', cursor: 'pointer' }}>✕</button>
                      )}
                    </div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                      {c.tailles.map((t, ti) => (
                        <div key={t.taille} onClick={() => updateTailleCouleur(ci, ti, 'active', !t.active)}
                          style={{ display: 'flex', alignItems: 'center', gap: '4px', background: t.active ? '#d4a853' : '#111', border: `1px solid ${t.active ? '#d4a853' : '#333'}`, borderRadius: '6px', padding: '5px 8px', cursor: 'pointer' }}>
                          <span style={{ fontSize: '12px', fontWeight: 600, color: t.active ? '#111' : '#555' }}>{t.taille}</span>
                          {t.active && (
              ...
              <button onClick={publierProduit} disabled={saving}
                style={{ width: '100%', padding: '14px', borderRadius: '8px', background: saving ? '#333' : '#d4a853', border: 'none', color: '#111', fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', fontSize: '15px', marginTop: '16px' }}>
        ...
        {onglet === 'fournisseurs' && (
          <div>
            <div style={{ background: '#111', border: '1px solid #222', borderRadius: '12px', padding: '20px', maxWidth: '500px', marginBottom: '20px' }}>
              <h3 style={{ margin: '0 0 16px', fontSize: '15px', color: '#d4a853' }}>🏭 Ajouter un fournisseur</h3>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
                <input value={fournisseurForm.nom} onChange={e => setFournisseurForm(p => ({ ...p, nom: e.target.value }))}
                  placeholder="Nom fournisseur *"
                  style={{ padding: '10px', borderRadius: '8px', background: '#1a1a1a', border: '1px solid #333', color: 'white', fontSize: '13px' }} />
                <input value={fournisseurForm.pays} onChange={e => setFournisseurForm(p => ({ ...p, pays: e.target.value }))}
                  placeholder="Pays"
                  style={{ padding: '10px', borderRadius: '8px', background: '#1a1a1a', border: '1px solid #333', color: 'white', fontSize: '13px' }} />
                <input value={fournisseurForm.telephone} onChange={e => setFournisseurForm(p => ({ ...p, telephone: e.target.value }))}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '10px' }}>
              {fournisseurs.map(f => (
                <div key={f.id} style={{ background: '#111', border: '1px solid #222', borderRadius: '10px', padding: '14px' }}>
                  <p style={{ margin: 0, fontWeight: 600, fontSize: '14px' }}>{f.nom}</p>
                  <p style={{ margin: '2px 0', color: '#888', fontSize: '12px' }}>📍 {f.pays}</p>
                  {f.telephone && <p style={{ margin: '2px 0', color: '#666', fontSize: '12px' }}>📞 {f.telephone}</p>}
                  <span style={{ fontSize: '11px', background: '#1a2035', color: '#378ADD', padding: '2px 8px', borderRadius: '10px' }}>{f.type}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <div style={{ background: '#111', border: '1px solid #222', borderRadius: '12px', padding: '20px', maxWidth: '500px', marginBottom: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <input value={achatForm.nom_produit} onChange={e => setAchatForm(p => ({ ...p, nom_produit: e.target.value }))}
                  placeholder="Nom produit *"
                  style={{ padding: '10px', borderRadius: '8px', background: '#1a1a1a', border: '1px solid #333', color: 'white', fontSize: '13px' }} />
                <div style={{ display: 'flex', gap: '10px' }}>
                  <input type="number" value={achatForm.quantite} onChange={e => setAchatForm(p => ({ ...p, quantite: Number(e.target.value) }))}
                    placeholder="Quantité"
                    style={{ flex: 1, padding: '10px', borderRadius: '8px', background: '#1a1a1a', border: '1px solid #333', color: 'white', fontSize: '13px' }} />
                  <input type="number" value={achatForm.prix_achat_unitaire} onChange={e => setAchatForm(p => ({ ...p, prix_achat_unitaire: Number(e.target.value) }))}
                ...
                <input type="date" value={achatForm.date_achat} onChange={e => setAchatForm(p => ({ ...p, date_achat: e.target.value }))}
                  style={{ padding: '10px', borderRadius: '8px', background: '#1a1a1a', border: '1px solid #333', color: 'white', fontSize: '13px' }} />
                <div style={{ background: '#0a1a12', borderRadius: '8px', padding: '10px', display: 'flex', justifyContent: 'space-between' }}>
            ...
            <h3 style={{ color: '#888', fontSize: '13px', textTransform: 'uppercase', marginBottom: '12px' }}>Derniers achats</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {achats.map(a => (
                <div key={a.id} style={{ background: '#111', border: '1px solid #222', borderRadius: '10px', padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <p style={{ margin: 0, fontWeight: 600, fontSize: '13px' }}>{a.nom_produit}</p>
                    <p style={{ margin: '2px 0 0', color: '#888', fontSize: '11px' }}>
                      🏭 {(a.fournisseurs as any)?.nom} · {a.quantite} pcs · {a.date_achat}
                    </p>
                  </div>
