...
export default function SuccesCataloguePage() {
  ...
  const getImage = (v: any) => v.image_url || produitSelectionne?.image_url || null

  function handleCommander() {
    if (selection.length === 0) return
    const query = new URLSearchParams({ produit: modele, taille, variantes: selection.join(',') })
    ...
    <div style={{ minHeight: '100vh', background: '#faf9f7', fontFamily: "'DM Sans', sans-serif", paddingBottom: 100 }}>
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 24 }}>
          ...
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#888', display: 'block', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 0.5 }}>2. Le modèle</label>
            <select value={modele} onChange={e => setModele(e.target.value)}
              style={{ width: '100%', padding: '12px 14px', borderRadius: 10, border: modele ? '1.5px solid #d4a853' : '1.5px solid #e5e2dc', fontSize: 15, color: modele ? '#1a1a1a' : '#aaa', background: '#fff', outline: 'none', cursor: 'pointer' }}>
              <option value="">Choisir...</option>
        ...
        {!taille || !modele ? (
          <div style={{ textAlign: 'center', padding: '48px 20px', color: '#bbb' }}>
            ...
            <p style={{ margin: 0, fontSize: 15 }}>Sélectionnez votre taille et un modèle</p>
          </div>
        ) : loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '48px 20px', color: '#bbb' }}>
            <div style={{ fontSize: 48, marginBottom: 12 }}>😔</div>
            <p style={{ margin: 0, fontSize: 15 }}>Aucune variante disponible en taille {taille}</p>
          </div>
        ) : (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 10 }}>
              {variantes.map(v => {
                ...
                return (
                  <button key={v.id} onClick={() => setSelection(prev => prev.includes(v.id) ? prev.filter(x => x !== v.id) : [...prev, v.id])}
                    style={{ background: '#fff', border: selected ? '2.5px solid #d4a853' : '1.5px solid #ece9e3', borderRadius: 12, padding: 0, cursor: 'pointer', overflow: 'hidden', position: 'relative', transform: selected ? 'scale(0.96)' : 'scale(1)', transition: 'transform 0.12s' }}>
                    <div style={{ width: '100%', aspectRatio: '3/4', background: 'linear-gradient(135deg, #f0ece4, #e8e1d5)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                      {imageUrl ? <img src={imageUrl} alt={v.couleur} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ fontSize: 36, opacity: 0.2 }}>✨</div>}
                    </div>
                    {selected && (
                      <div style={{ position: 'absolute', top: 8, right: 8, width: 24, height: 24, borderRadius: '50%', background: '#d4a853', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: '#111', fontWeight: 700 }}>✓</div>
                    )}
      ...
      {selection.length > 0 && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#fff', borderTop: '1px solid #ece9e3', padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ flex: 1 }}>
            <p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: '#1a1a1a' }}>{selection.length} article{selection.length > 1 ? 's' : ''} sélectionné{selection.length > 1 ? 's' : ''}</p>
